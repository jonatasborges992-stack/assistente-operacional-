...
window.interpretSmart=function(){
 ...
 if(!text){if(out)out.innerHTML='<div class="result warn"><b>⚠️ Digite ou fale o lançamento.</b></div>';return;}
 try{
   ...
   out.innerHTML=`<div class="result ${missing.length?'warn':'good'}">
     ...
     ${missing.length?`<p class="muted"><b>Para registrar:</b> ${esc(missing.join(' • '))}. Você pode corrigir diretamente o texto acima e interpretar novamente.</p>`:`<div class="actions"><button class="green" id="aiConfirm">✓ Confirmar e registrar</button><button class="secondary" id="aiCorrect">✎ Corrigir texto</button></div>`}
   </div>`;
   if(!missing.length){
 ...
 }catch(err){console.error('OPERA ONE interpreter',err);out.innerHTML='<div class="result bad"><b>⚠️ Erro interno de interpretação.</b><p>Edite o texto e tente novamente.</p></div>';}
};
})();
